...
* `If` statements execute block code when a condition is `met/true`.
* The `if block` start in `4` spaces characters from the beginning of a new line

 ###### 📄 template #1: (not recommended) 4 spaces from `:` character 
...
```
 ...
 ###### 📄 template #2: if block - 4 spaces in a new line
...
* Every line after that will continue the `if` block, if code start `4` spaces characters from the beginning of new line.
* if no spaces exist in that line, it will be execute outside the if block.

 ###### 📄 template : `if block` (3 lines block)
```
if (condition):
    print("Start Block");
    print("4 x spaces");
...
if age < 2:
    status = "Baby"
    print(f"{status} is: {age}");

if age > 2:
    status = "Not a Baby"
    print(f"{status} is: {age}");
    
print("Not in if() block, and it will be printed all the time");
```
...
```
Baby is: 1
Not in if() block, and it will be printed all the time
```
